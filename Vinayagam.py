...
def count_sp_edges(network, sources, terminals):
    ...
    for p, pair in tqdm(enumerate(product(all_sources, all_terminals)), total=(len(all_sources)* len(all_terminals))):
...
def infer_vinayagam(train_features, train_labels, test_features, test_labels, source_types=None):
    ...
    # precision, recall, deep_thresholds = precision_recall_curve(test_labels, probs[:, 1])
    # vinayagam_auc = auc(recall, precision)
