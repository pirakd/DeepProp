...
for percentile_idx, percentile in enumerate(percentiles):
    ...
    for prediction in prediction_types:
        d2d_scores.append(d2d_results_dict['results'][prediction]['overall']['directed_network']['percentiles'][percentile_idx])
        deep_scores.append(deep_results_dict['results'][prediction]['overall']['directed_network']['percentiles'][percentile_idx])
        undirected_scores.append(deep_results_dict['results'][prediction]['overall']['undirected_network']['percentiles'][percentile_idx])
    ...
    ax.set_title('Fraction of cancer driver genes among top {}% ranked genes'.format(percentile), fontsize=fontsize_1)
    ax.set_ylim()
