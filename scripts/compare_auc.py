...
from old_utils import read_data, generate_feature_columns, normalize_features, get_pulling_func, get_root_path
import torch
...
input_file = path.join(get_root_path(), 'DrugsInfo', 'input')
...
train_features_2, test_features_2 = features_2[train_indexes], features_2[test_indexes]
probs_2, d2d_precision_2, d2d_recall_2, d2d_auc_2 = eval_D2D_2(train_features_2, test_features_2)


features = generate_D2D_features(source_features, terminal_features)
train_features, test_features = features[train_indexes], features[test_indexes]
probs, d2d_precision, d2d_recall, d2d_auc = eval_D2D(train_features, test_features)
...
best_eval_loss, best_acc, deep_auc, best_epoch, deep_preicison, deep_recall = trainer.train(train_loader=train_loader, eval_loader=eval_loader, model=model)

plt.plot([0, 1], [0.5, 0.5], '--', color=(0.8, 0.8, 0.8), label='random')
plt.plot(deep_recall, deep_preicison, 'o--', marker='o', label='DeepSetPropagation (%0.2f)' % deep_auc, lw=2, markersize=3)  # , markeredgecolor = 'dimgrey')
plt.plot(d2d_recall, d2d_precision, 'o--', marker='o', label='Diffusion2Direction (%0.2f)' % d2d_auc, lw=2, markersize=3)  # , markeredgecolor = 'dimgrey')
