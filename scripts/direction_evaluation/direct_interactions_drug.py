...
for experiment_type in experiments_types:
    ...
    propagation_scores, col_id_to_idx = propagate_directed_network(undirected_network=network,
                                                                                  directed_edges=directed_interactions_flipped,
                                                                                  sources=terminals, args=args)
    ...
    source_ranks = []
    single_input_results_dict = {}
    for exp_idx, exp_name in enumerate(sources.keys()):
        drug_targets_gene_indexes = [col_id_to_idx[x] for x in sources[exp_name] if x in col_id_to_idx]
        ...
        undirected_source_ranks.append(rank_undirected[drug_targets_gene_indexes])
        directed_source_ranks.append(rank_directed[drug_targets_gene_indexes])

    total_source_genes = np.sum([len(x) for x in sources.values()])
