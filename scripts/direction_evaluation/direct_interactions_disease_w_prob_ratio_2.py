...
import numpy as np
from utils import read_network, propagate_directed_network, propagate_partially_directed_network, redirect_output, get_time, train_test_split
...
for name in prediction_types:
    ...
    predicted_edges = {x: predictions_dict[x] / (predictions_dict[(x[1], x[0])] + 1e-12) for x in
                       predictions_dict.keys()}
    prediction['direction_prob'].loc[predicted_edges.keys()] = list(predicted_edges.values())
    predictions.append(prediction)

predictions = pd.concat(predictions)
ratio_means  = predictions.groupby(level=[0, 1])['direction_prob'].mean()
ratio_means = ratio_means[ratio_means<1]
ratio_means = ratio_means.to_dict()
ratio_means_flipped = {(key[1], key[0]):value for key,value in ratio_means.items()}
...
partially_directed_propagation_scores, _ = propagate_partially_directed_network(undirected_network=network,
                                                            prob_ratios=ratio_means_flipped,
                                                            sources=target_disease_genes, args=args)
...
directed_test_ranks = []
directed_ranks_list = []
p_directed_test_ranks = []
p_directed_ranks_list = []
auc_list = []
...
for e, exp in enumerate(source_disease_genes.keys()):
    ...
    p_directed_ranks = np.argsort(np.argsort(partially_directed_propagation_scores[e]))
    p_directed_ranks_list.append(p_directed_ranks)
...
p_directed_ranks_list = np.array([x for xx in p_directed_ranks_list for x in xx])
p_directed_percent_of_cancer_genes_of_percentile = [
    np.sum(np.array(p_directed_ranks_list[labels_list == 1 ]) > (len(partially_directed_propagation_scores[0]) - x)) / (x * len(target_disease_genes.keys())) for x in num_genes_in_source_precentile]
p_directed_fold_enrichment = [x / dataset_fraction_of_total_genes for x in p_directed_percent_of_cancer_genes_of_percentile]
...
precision, recall, _ = precision_recall_curve(labels_list, p_directed_ranks_list)
p_directed_auc = auc(recall, precision)
...
results_dict['results'] = \
    ...
    'partially_directed_network': {'auc': p_directed_auc,
                          'percentiles':p_directed_percent_of_cancer_genes_of_percentile,
                          'fold_enrichment': p_directed_fold_enrichment},}
