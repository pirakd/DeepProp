...
go_columns = {'symbol':2 ,'go_id': 4, 'go_aspect':8}

go_file_path = path.join(root_path, 'input', 'other', 'goa_human.gaf')
go_data = pd.read_csv(go_file_path, sep='\t', usecols=[go_columns['symbol'], go_columns['go_id'], go_columns['go_aspect']], comment='!', header=None)

# cancer_genes_dict['overall'] = set(x for xx in symbol_to_entrez_id_dict.values() for x in xx)
...
go_unique_genes = np.unique(go_data[go_columns['symbol']].to_numpy().astype(str))
symbol_to_entrez_id_dict = translator.translate(go_data[go_columns['symbol']], 'symbol', 'entrez_id')
symbol_to_entrez_id_dict = {x:xx for x,xx in symbol_to_entrez_id_dict.items() if xx in graph.nodes}

go_dict = {'F':defaultdict(set),
           'C':defaultdict(set),
           'P':defaultdict(set)}
for idx, row in go_data.iterrows():
    go_id, aspect, symbol  = row[go_columns['go_id']], row[go_columns['go_aspect']], row[go_columns['symbol']]
    if symbol in symbol_to_entrez_id_dict:
        go_dict[aspect][go_id].add(symbol_to_entrez_id_dict[symbol])

for aspect in go_dict.keys():
    for go_term in list(go_dict[aspect].keys()):
        if 1000 < len(go_dict[aspect][go_term]) or len(go_dict[aspect][go_term]) < 20:
            del go_dict[aspect][go_term]
...
for aspect_name, aspect_terms in go_dict.items():
    train_genes = {}
    test_genes = {}
    count = 0
    for term_name, term_genes in aspect_terms.items():
        if count>500:
            break
        term_genes_array = np.array(list(term_genes))
        for train_index, test_index in kf.split(term_genes_array):

            train_genes[count] = term_genes_array[train_index]
            test_genes[count] = term_genes_array[test_index]
            count += 1

    directed_propagation_scores, col_id_to_idx = propagate_directed_network(undirected_network=network,
                                                                directed_edges=consensus_predictions,
                                                                sources=train_genes, args=args)
    undirected_propagation_scores, col_id_to_idx = propagate_directed_network(undirected_network=network,
                                                                directed_edges=[],
                                                                sources=train_genes, args=args)
    num_genes_in_source_precentile = [len(directed_propagation_scores[0]) * x * 0.01 for x in [0.25, 0.5, 1, 5, 10]]
    directed_test_ranks = []
    auc_list = []
    for exp in train_genes.keys():
        directed_source_ranks = [], []
        directed_source_scores = [], []
        source_ranks = []

        test_gene_indexes = [col_id_to_idx[x] for x in test_genes[exp] if x in col_id_to_idx]
        directed_ranks = np.argsort(np.argsort(directed_propagation_scores[exp]))
        undirected_ranks = np.argsort(np.argsort(undirected_propagation_scores[exp]))

        #TODO continue from here
        undirected_percent_of_cancer_genes_of_percentile = [
            np.sum(np.array(undirected_ranks) > len(directed_propagation_scores[0]) - x) / (x * len(tr)) for x in
